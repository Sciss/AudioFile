...
object AudioFileType extends AudioFileTypePlatform {
  ...
  sealed trait CanIdentify extends AudioFileType {
    ...
    private[audiofile] def identify(dis: DataInputStream): Boolean

//    private[audiofile] def identifyAsync(ch: AsyncReadableByteChannel): Future[Boolean]
  }

  trait CanRead extends AudioFileType with CanReadPlatform {
    @throws(classOf[IOException])
    private[audiofile] def read(dis: DataInputStream): AudioFileHeader
  ...
  private[audiofile] def register(fileType: AudioFileType): Unit = {
    allVar :+= fileType
