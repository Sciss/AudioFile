...
ScalaAudioFile is a Scala library to read and write audio files. It is (C)opyright 2004-2011 by Hanns Holger Rutz. All rights reserved. ScalaAudioFile is released under the "GNU General Public License":http://github.com/Sciss/ScalaAudioFile/blob/master/licenses/ScalaAudioFile-License.txt and comes with absolutely no warranties. To contact the author, send an email to @contact at sciss.de@

h3. requirements / installation

ScalaAudioFile currently compiles against Scala 2.9.0 and 2.8.1. The repository contains a project file for Intellij IDEA 10 CE, and project files for sbt. Targets for sbt:
...
Currently, the only formats supported are AIFF (read, write), WAVE (read), and IRCAM (read, write)!
...
To the user, frame data is always represented as de-interleaved 32-bit floating point data, so you create a user buffer through @Array.ofDim[ Float ]( numChannels, bufFrames )@, or use a convenience method such as @AudioFile.frameBuffer(...)@.
